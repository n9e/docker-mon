...
func updateContainerStat() {
	datas, err := getContainersData()
	if err != nil {
		logger.Errorf("get containers data err:%v", err)
		return
	}
	now = time.Now().Unix()
...
func collectDockerStats(container v1.ContainerInfo) {
	stats := container.Stats
	if len(container.Stats) < 2 {
		logger.Error("container.Stats num < 2")
		return
	}
	inspect, err := getInspect(container.Id)
	if err != nil {
	...
	if containerName == "POD" {
		...
		if ip, exists := HostIp.Get(hostname + "."); exists {
			containerData.ContainerDataAppend(netMetric(stats, nid, ip)...)
		} else if ip, exists := HostIp.Get(hostname); exists {
			containerData.ContainerDataAppend(netMetric(stats, nid, ip)...)
		}
		return
...
func getInspect(containerId string) (containerInspect *ContainerInspect, err error) {
		...
		if envs != nil {
			for i := 0; i < len(envs); i++ {
				if strings.Contains(envs[i], "CCP_POD_IP") {
					...
					HostIp.Set(inspect.Config.Hostname, containerInspect.Host)
				}
			}
		}

		if containerInspect.Host == "" {
			containerInspect.Host, _ = HostIp.Get(inspect.Config.Hostname)
		}
	}
...
func getDial(netwrok, addr string) (net.Conn, error) {
	sockFile := config.G.DockerSockFile
...
func toDockerMetric(name, nid, endpoint string, value interface{}, tags map[string]string) Metric {
	ret := Metric{Nid: nid, Metric: name, ValueUntyped: value, TagsMap: map[string]string{}, Step: int64(config.G.Step), Timestamp: now, CounterType: config.GAUGE}
	for k, v := range tags {
		ret.TagsMap[k] = v
	}
...
func getContainersData() (containerData []v1.ContainerInfo, err error) {
	clientCon, err := client.NewClient(config.G.CadvisorUrl)
	if err != nil {
		return
	}
